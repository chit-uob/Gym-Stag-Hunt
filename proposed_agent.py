...
def distance_delta(old_location, new_location, target_position):
    """
...
class ProposedAgent:
    ...
    def update_parameters(self, old_obs, new_obs):
        ...
        self.stag_weight += distance_delta(self_location, new_self_location, stag_location) * self.stag_multiplier
        self.plant_weight += distance_delta(self_location, new_self_location, plant_location_1) * self.plant_multiplier
        self.plant_weight += distance_delta(self_location, new_self_location, plant_location_2) * self.plant_multiplier
        self.player_weight += distance_delta(self_location, new_self_location, other_player_location) * self.player_multiplier
    ...
    def calculate_action_reward(self, old_location, new_location, stag_location, plant_location_1, plant_location_2, other_player_location):
        ...
        stag_change = distance_delta(old_location, new_location, stag_location)
        plant_change = distance_delta(old_location, new_location, plant_location_1) + distance_delta(old_location, new_location, plant_location_2)
        player_change = distance_delta(old_location, new_location, other_player_location)
        # return the reward
